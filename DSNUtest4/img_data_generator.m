function [x_list, y_list] = img_data_generator(I, b1, b2, PIC_MAX_ROW, PIC_MAX_COL, SAMP_ROW_LEN, SAMP_COL_LEN)
%IMG_X_GENERATER 此处显示有关此函数的摘要
%   此处显示详细说明

    x_list = zeros(2, PIC_MAX_COL);
    y_list = zeros(2 * SAMP_ROW_LEN, PIC_MAX_COL);
    ...
    y_list(1 : SAMP_ROW_LEN, :) = I_pt1(1 : SAMP_ROW_LEN, :);
    y_list(SAMP_ROW_LEN + 1 : end, :) = I_pt2(1 : SAMP_ROW_LEN, :);
    ...
    for i = BORDER_WID + 1 : PIC_MAX_COL - BORDER_WID
        x_list(1, i) = mean(mean(I_pt1(1 : SAMP_ROW_LEN, i - BORDER_WID + 1 : i + BORDER_WID - 1)));
        x_list(2, i) = mean(mean(I_pt2(1 : SAMP_ROW_LEN, i - BORDER_WID + 1 : i + BORDER_WID - 1)));
    end
